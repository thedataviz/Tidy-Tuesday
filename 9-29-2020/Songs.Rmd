...
state_perc <- state_results %>% mutate(
  ...
  mutate(
    ...
    Value = ifelse(`Taylor Swift`>`Beyoncé`,`Taylor Swift`,`Beyoncé`)
  )%>%
...
```{r message=FALSE, warning=FALSE}
plot_usmap(data = state_perc, values = "Value",color="grey") + 
  scale_fill_manual(
    values = c(beyonce_color,taylor_color), name = "Artist"
